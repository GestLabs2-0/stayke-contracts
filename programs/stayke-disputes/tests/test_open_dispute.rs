...
use {
    anchor_lang::{
        solana_program::instruction::Instruction, AnchorDeserialize,
...
fn open_dispute_guest_success() {
    ...
    setup_booking(
        &mut svm, booking_key,
        guest_profile_pda, host_profile_pda, property,
    ...
    assert!(res.is_ok(), "Expected successful open_dispute, got: {:?}", res.err());
    ...
    assert_eq!(disp.initiator, guest.pubkey());
    assert_eq!(disp.guilty, host.pubkey());
    assert!(matches!(disp.reason, DisputeReason::PropertyNotAsDescribed));
    assert!(matches!(disp.status, stayke_disputes::state::DisputeStatus::Open));
    assert!(disp.created_at > 0);
...
fn open_dispute_host_success() {
    ...
    setup_booking(&mut svm, booking_key, guest_profile_pda, host_profile_pda, property, BookingStatus::Active);
    ...
    let res = svm.send_transaction(tx);
    assert!(res.is_ok(), "Expected successful open_dispute by host, got: {:?}", res.err());

    let account = svm.get_account(&dispute).unwrap();
    let disp: stayke_disputes::state::Dispute =
        AnchorDeserialize::deserialize(&mut &account.data[8..]).unwrap();
    assert_eq!(disp.initiator, host.pubkey());
    assert_eq!(disp.guilty, guest.pubkey());
...
fn open_dispute_booking_not_active_fails() {
    ...
    setup_global_config(&mut svm, stayke_disputes::id(), stayke_escrow::id());
    setup_user_profile(&mut svm, guest.pubkey());
    setup_booking(&mut svm, booking_key, guest_profile_pda, host_profile_pda, property, BookingStatus::Pending);
    ...
    let instruction = Instruction::new_with_bytes(
        ...
        stayke_disputes::accounts::OpenDispute {
            payer: payer.pubkey(), initiator: guest.pubkey(), initiator_profile: guest_profile_pda,
            booking: booking_key, dispute, cpi_authority: cpi_auth, global_config: global_cfg,
            stayke_escrow_program: stayke_escrow::id(),
            system_program: solana_system_program::id(),
        }.to_account_metas(None),
    );
    ...
    assert_eq!(res.unwrap_err().err,
        TransactionError::InstructionError(0,
            Custom(u32::from(stayke_disputes::error::DisputeError::BookingNotActive))));
}
...
fn open_dispute_stranger_fails() {
    ...
    let instruction = Instruction::new_with_bytes(
        ...
        stayke_disputes::accounts::OpenDispute {
            payer: payer.pubkey(), initiator: stranger.pubkey(),
    ...
    let blockhash = svm.latest_blockhash();
    let msg = Message::new_with_blockhash(&[instruction], Some(&payer.pubkey()), &blockhash);
    let tx = VersionedTransaction::try_new(VersionedMessage::Legacy(msg), &[&payer, &stranger]).unwrap();
    let res = svm.send_transaction(tx);
    assert!(res.is_err());
    assert_eq!(res.unwrap_err().err,
        TransactionError::InstructionError(0,
            Custom(u32::from(stayke_disputes::error::DisputeError::UnauthorizedDisputeInitiator))));
}
...
fn open_dispute_unverified_user_fails() {
    ...
    assert_eq!(res.unwrap_err().err,
        TransactionError::InstructionError(0,
            Custom(u32::from(stayke_disputes::error::DisputeError::UserNotVerified))));
}
