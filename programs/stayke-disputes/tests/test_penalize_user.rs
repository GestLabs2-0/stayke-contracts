...
fn penalize_user_unauthorized_admin_fails() {
    ...
    let affected_wallet = Pubkey::new_unique();
    make_mint(&mut svm, usdc_mint);
    make_token_account(&mut svm, vault_ata, usdc_mint, Pubkey::new_unique());
    make_token_account(&mut svm, affected_ata, usdc_mint, affected_wallet);
...
#[test]
fn penalize_user_foreign_token_account_fails() {
    let (mut svm, payer) = build_svm_with_programs();
    let admin = Keypair::new();
    let penalized = Pubkey::new_unique();
    svm.airdrop(&admin.pubkey(), 1_000_000_000).unwrap();
    ...
    svm.set_account(
        ...
        solana_account::Account {
            ...
            data: to_account_data(
                "GlobalConfig",
                &stayke_config::state::GlobalConfig {
                    authority: Pubkey::new_unique(),
                    minimum_deposit: 100_000,
                    fee_bps: 200,
                    usdc_mint,
    ...
    let instruction = anchor_lang::solana_program::instruction::Instruction::new_with_bytes(
        ...
        stayke_disputes::accounts::PenalizeUser {
            ...
            .0,
            penalized_user_profile: Pubkey::find_program_address(
                &[
                    stayke_core::constants::USER_PROFILE_SEED.as_bytes(),
                    penalized.as_ref(),
                ],
    ...
    assert_eq!(
        ...
        TransactionError::InstructionError(
            0,
            Custom(u32::from(
                stayke_disputes::error::DisputeError::InvalidAffectedTokenAccount
            ))
        )
    );
}
...
#[test]
fn penalize_user_foreign_reputation_fails() {
    let (mut svm, payer) = build_svm_with_programs();
    let admin = Keypair::new();
    let penalized = Pubkey::new_unique();
    svm.airdrop(&admin.pubkey(), 1_000_000_000).unwrap();
    ...
    setup_user_profile(&mut svm, penalized);
    let foreign_reputation = setup_reputation_profile(&mut svm, Pubkey::new_unique());
    ...
    let instruction = anchor_lang::solana_program::instruction::Instruction::new_with_bytes(
        ...
        stayke_disputes::accounts::PenalizeUser {
            ...
            .0,
            penalized_reputation_profile: foreign_reputation,
            affected_token_account: affected_ata,
            affected_wallet,
            treasury_config: treasury_config_pda,
            global_config: global_cfg_pda,
    ...
    assert_eq!(
        ...
        TransactionError::InstructionError(
            0,
            Custom(anchor_lang::error::ErrorCode::ConstraintSeeds.into())
        )
    );
}
*/
