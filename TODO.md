...
| 9 | `programs/stayke-escrow/src/instructions/host_reject_booking.rs` | 36 | **check if lamports for closing booking goes to payer vs original creator** — Evaluar política de reembolso de renta al cerrar cuentas (`Booking`, `EscrowTokenAccount`): ¿guardar `payer: Pubkey` en `Booking` para devolverlo al creador original, o usar el `payer` de la transacción de cierre como incentivo para relayers/crank? Ver guía [stayke-todos-security.guide.md](./docs/stayke-todos-security.guide.md). | 🟡 HIGH |
